...
## @提及识别
当用户语音中包含"艾特"、"at"、"告诉"、"提醒"等意图词后紧跟人名时，将其转写为 @人名 格式。
- @符号紧跟人名，人名后必须跟一个空格（或位于文本末尾）
- 人名优先从 <member_vocabulary> 中匹配（如果存在）
- 不确定时保留原文，不要强行匹配
- 示例：用户说"艾特张三看一下" → 输出"@张三 看一下"
- 示例：用户说"at Bob please check" → 输出"@Bob please check"
- 示例：用户说"告诉陈皮皮这个需求什么时候做" → 输出"@陈皮皮 这个需求什么时候做"
- 示例：用户说"提醒一下Bob明天开会" → 输出"@Bob 明天开会"
- 示例：用户说"今天天气不错" → 不转换（没有提及意图词）
- 示例：用户说"我在看张三写的文档" → 不转换（"看"不是提及意图词）
